...
| `TRUST_PROXY` | no, detected | See below. Set it only to override the detection |
| `BEHIND_TLS` | no, detected | Turns on HSTS. Detected alongside the proxy |
...
`TRUST_PROXY` used to fail quietly, which is worse, so it is now detected
rather than declared. Every managed host puts a proxy in front of your
container, and without this every request in the world arrives from the proxy's
address and shares one rate-limit allowance: nothing errors, the app simply
starts refusing people, and the first busy user locks out everyone else.

Render, Railway, Fly and Heroku each set a variable of their own that a request
cannot reach, so the app reads one of those and trusts the forwarded address on
its own. `BEHIND_TLS` follows the same signal, since those are the same hosts
that terminate TLS.

Setting either explicitly still wins. Do that on a host not in that list, and
only where a proxy really is in front: where nothing is proxying, the forwarded
header is written by whoever is calling, and trusting it hands every caller a
fresh allowance per request, which is the same as having no limit. The app
still says so in its log the first time it sees a forwarded header it is not
trusting.
